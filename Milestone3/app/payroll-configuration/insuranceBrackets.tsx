...
type InsuranceStatus = ConfigStatus;

const normalizeStatus = (
  status?: InsuranceStatus | string | null
): InsuranceStatus => {
  const normalized = (status || "draft").toString().toLowerCase();
  if (normalized === "approved" || normalized === "rejected") {
    return normalized as InsuranceStatus;
  }
  return "draft";
};

const statusBadgeColor = (status: InsuranceStatus) => {
  switch (status) {
    case "approved":
...
const formatStatus = (status: InsuranceStatus) =>
...
export default function InsuranceBrackets() {
  ...
  const selectedBracketStatus = selectedBracket
    ? normalizeStatus(selectedBracket.status)
    : null;
