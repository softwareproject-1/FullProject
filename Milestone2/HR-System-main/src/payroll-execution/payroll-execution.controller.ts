...
export class PayrollExecutionController {
    ...
    @Roles('PAYROLL_SPECIALIST', 'PAYROLL_MANAGER') // Phase 0: Specialists review benefits before run
    ...
    @Post('events/check')
    @Roles('PAYROLL_SPECIALIST', 'PAYROLL_MANAGER') // Phase 0: Check employee HR events
    ...
    @Post('period')
    @Roles('PAYROLL_SPECIALIST') // Phase 1: Only Specialists can initiate payroll runs
    ...
    @Patch('period/review')
    @Roles('PAYROLL_SPECIALIST') // Phase 1: Specialists approve/reject payroll period
    ...
    @Roles('PAYROLL_SPECIALIST', 'PAYROLL_MANAGER', 'FINANCE_STAFF') // Phase 1.1: All payroll roles can view draft
    ...
    @Roles('PAYROLL_SPECIALIST') // Phase 1.1.B: Only Specialists can trigger salary calculations
    ...
    @Roles('PAYROLL_SPECIALIST', 'PAYROLL_MANAGER') // Manual adjustments require Specialist or Manager authority
    ...
    @Get('test/db-status')
    @Roles('PAYROLL_MANAGER', 'FINANCE_STAFF') // Debug: Only senior staff can check database status
    ...
    @Roles('PAYROLL_SPECIALIST') // Phase 2: Specialists submit for manager review after validation
    ...
    @Roles('PAYROLL_MANAGER') // Phase 3: Only Managers can approve/reject payroll runs
    ...
    @Roles('FINANCE_STAFF') // Phase 3: Finance Staff final approval before execution
    ...
    @Get('runs/:id/preview')
    @Roles('PAYROLL_SPECIALIST', 'PAYROLL_MANAGER', 'FINANCE_STAFF') // REQ-PY-6: Preview dashboard for all payroll roles
    ...
    @Patch('runs/:id/lock')
    @Roles('PAYROLL_MANAGER') // REQ-PY-7: Only Managers can lock/freeze payroll runs
    ...
    @Roles('PAYROLL_MANAGER') // REQ-PY-19: Only Managers can unfreeze with justification
